...
RegioningPlugin::RegioningPlugin(QWidget *parent) : MainWindowTab(parent), create_debug_images(true) {
	...
	/* Create actions */
	this->save_view = new QAction(QIcon(":/img/icon_save.svg"), tr("Save Regions for this View"), this);
...
/*
			if (currentq.getID() == 1)
				drawPoly(&currentq, QColor(0,0,204,125));
			if (currentq.getID() == 2)
				drawPoly(&currentq, QColor(204,0,0,125));
			if (currentq.getID() == 3)
				drawPoly(&currentq, QColor(102,0,102,125));*/
		...
		/*else {
			...
			if (q2->isComplete()) {
			if (q2->getID() == 0)
				drawPoly(q2, QColor(0,204,0,125));
			if (q2->getID() == 1)
				drawPoly(q2, QColor(0,0,204,125));
			if (q2->getID() == 2)
				drawPoly(q2, QColor(204,0,0,125));
			if (q2->getID() == 3)
				drawPoly(q2, QColor(102,0,102,125));
			}
		    }
		}*/
	}
